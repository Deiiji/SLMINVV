...
namespace SLMIV.Utils
{
        ...
        public class Common
        {
            public static string MainLog = Application.StartupPath + "\\mainlog.txt";
            public static string ImageLog = Application.StartupPath + "\\imagelog.txt";
            ...
            public static string urlShop = @"http://shop.onrez.com/Seneca_Taliaferro";
            public static string urlSupport = @"http://code.google.com/p/slmiv/issues/list";
            public static string TempInv= Application.StartupPath + "\\tempinv.inv";
            public static string invFolder = Application.StartupPath + "\\inv";
        ...
        public class MIUtils
        {
            ...
            {//http://www.devhood.com/tutorials/tutorial_details.aspx?tutorial_id=773
                ...
                foreach (TreeNode tn0 in treeViewOne.Nodes)
                {
                    //First one should be My Inventory, so create it
                    //tn0.Name = uidkey
                    //tn0.Text = title

                    xmlWriter.WriteStartElement(Convert.ToString(tn0.Tag));//My Inventory?
                    xmlWriter.WriteAttributeString("name", tn0.Text);
                    xmlWriter.WriteAttributeString("uuid", tn0.Name);
                    ImageWriteAttributeString(xmlWriter, tn0);
                    //xmlWriter.WriteValue(Utils.MIUtils.GetMyInventoryObjectByText(tn0.Text, out itype).ToString());
                    xmlWriter.WriteValue(Utils.MIUtils.GetInvObj(tn0.Name, MIUtils.OD_treeindex, out itype).ToString());
                    foreach (TreeNode tn1 in tn0.Nodes)
                    {
                        //XmlElement childNode1 = xmlDoc.CreateElement(tn1.Name);
                        xmlWriter.WriteStartElement(Convert.ToString(tn1.Tag));
                        xmlWriter.WriteAttributeString("name", tn1.Text);
                        xmlWriter.WriteAttributeString("uuid", tn1.Name);
                        ImageWriteAttributeString(xmlWriter, tn1);
                        xmlWriter.WriteValue(Utils.MIUtils.GetInvObj(tn1.Name, MIUtils.OD_treeindex, out itype).ToString());
                        foreach (TreeNode tn2 in tn1.Nodes)
                        {
                            xmlWriter.WriteStartElement(Convert.ToString(tn2.Tag));
                            xmlWriter.WriteAttributeString("name", tn2.Text);
                            xmlWriter.WriteAttributeString("uuid", tn2.Name);
                            ImageWriteAttributeString(xmlWriter, tn2);
                            xmlWriter.WriteValue(Utils.MIUtils.GetInvObj(tn2.Name, MIUtils.OD_treeindex, out itype).ToString());
                            foreach (TreeNode tn3 in tn2.Nodes)
                            {
                                xmlWriter.WriteStartElement(Convert.ToString(tn3.Tag));
                                xmlWriter.WriteAttributeString("name", tn3.Text);
                                xmlWriter.WriteAttributeString("uuid", tn3.Name);
                                ImageWriteAttributeString(xmlWriter, tn3);
                                xmlWriter.WriteValue(Utils.MIUtils.GetInvObj(tn3.Name, MIUtils.OD_treeindex, out itype).ToString());
                                foreach (TreeNode tn4 in tn3.Nodes)
                                {
                                    xmlWriter.WriteStartElement(Convert.ToString(tn4.Tag));
                                    xmlWriter.WriteAttributeString("name", tn4.Text);
                                    xmlWriter.WriteAttributeString("uuid", tn4.Name);
                                    ImageWriteAttributeString(xmlWriter, tn4);
                                    xmlWriter.WriteValue(Utils.MIUtils.GetInvObj(tn4.Name, MIUtils.OD_treeindex, out itype).ToString());
                                    foreach (TreeNode tn5 in tn4.Nodes)
                                    {
                                        xmlWriter.WriteStartElement(Convert.ToString(tn5.Tag));
                                        xmlWriter.WriteAttributeString("name", tn5.Text);
                                        xmlWriter.WriteAttributeString("uuid", tn5.Name);
                                        ImageWriteAttributeString(xmlWriter, tn5);
                                        xmlWriter.WriteValue(Utils.MIUtils.GetInvObj(tn5.Name, MIUtils.OD_treeindex, out itype).ToString());
                                        foreach (TreeNode tn6 in tn5.Nodes)
                                        {
                                            xmlWriter.WriteStartElement(Convert.ToString(tn6.Tag));
                                            xmlWriter.WriteAttributeString("name", tn6.Text);
                                            xmlWriter.WriteAttributeString("uuid", tn6.Name);
                                            ImageWriteAttributeString(xmlWriter, tn6);
                                            xmlWriter.WriteValue(Utils.MIUtils.GetInvObj(tn6.Name, MIUtils.OD_treeindex, out itype).ToString());
                                            foreach (TreeNode tn7 in tn6.Nodes)
                                            {
                                                xmlWriter.WriteStartElement(Convert.ToString(tn7.Tag));
                                                xmlWriter.WriteAttributeString("name", tn7.Text);
                                                xmlWriter.WriteAttributeString("uuid", tn7.Name);
                                                ImageWriteAttributeString(xmlWriter, tn7);
                                                xmlWriter.WriteValue(Utils.MIUtils.GetInvObj(tn7.Name, MIUtils.OD_treeindex, out itype).ToString());
                                                foreach (TreeNode tn8 in tn7.Nodes)
                                                {
                                                    xmlWriter.WriteStartElement(Convert.ToString(tn8.Tag));
                                                    xmlWriter.WriteAttributeString("name", tn8.Text);
                                                    xmlWriter.WriteAttributeString("uuid", tn8.Name);
                                                    ImageWriteAttributeString(xmlWriter, tn8);
                                                    xmlWriter.WriteValue(Utils.MIUtils.GetInvObj(tn8.Name, MIUtils.OD_treeindex, out itype).ToString());
                                                }
            ...
            private void InsertUnderParent(string parent_id, string idkey, string title, int imageindex, string elementname, TreeView treeViewOne)
            {
                foreach (TreeNode tn0 in treeViewOne.Nodes)
                {
