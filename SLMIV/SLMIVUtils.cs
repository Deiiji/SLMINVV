...
namespace SLMIV.Utils
{
    ...
    public class MIUtils
    {
        ...
        public static List<QueuedDownloadInfo> RemoveDupQDI(List<QueuedDownloadInfo> input)
        {
            List<QueuedDownloadInfo> NewQDIList = new List<QueuedDownloadInfo>();
            List<String> FileNameList = new List<string>();
            foreach (QueuedDownloadInfo qdi in input)
            {
                if (!FileNameList.Contains(Path.GetFileName(qdi.FileName)))
                {
                    FileNameList.Add(Path.GetFileName(qdi.FileName));
                    NewQDIList.Add(qdi);
                }
            }
            return NewQDIList;
